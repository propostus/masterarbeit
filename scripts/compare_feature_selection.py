# scripts/compare_feature_selection.py
import os
import argparse
import pandas as pd
import numpy as np
...
def evaluate_rf(dataset_path, n_splits=5):
    ...
    return {
        ...
        "strategy": detect_strategy(dataset_path),
        "features": X.shape[1],
...
def compare_feature_selection(datasets, out_csv, n_splits=5):
    results = []
    for ds in datasets:
        print(f"==> Evaluating RandomForest on {ds}")
        res = evaluate_rf(ds, n_splits=n_splits)
        results.append(res)
        print(f"  {res['strategy']} ({res['features']} Features): "
              f"R2={res['r2_mean']:.4f}, MAE={res['mae_mean']:.4f}, RMSE={res['rmse_mean']:.4f}")
...
if __name__ == "__main__":
    ...
    compare_feature_selection(args.datasets, args.out_csv, n_splits=args.n_splits)