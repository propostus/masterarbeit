# scripts/compare_models_sigmos_wavlm.py
import os
import argparse
import numpy as np
import pandas as pd
...
def optimize_model(model_name, X_train, X_val, y_train, y_val, trials=15):
...
def compare_models(dataset_path, target_col, out_dir, trials=15):
    print(f"=== Modellvergleich für {target_col} auf Basis von {dataset_path} ===")
    df = pd.read_csv(dataset_path)
    print(f"Datensatz geladen: {df.shape[0]} Zeilen, {df.shape[1]} Spalten")

    X = df.drop(columns=[target_col, "filename"], errors="ignore")
    y = df[target_col]
...
def main():
    parser = argparse.ArgumentParser(description="Vergleiche verschiedene Modelle auf SigMOS+WavLM-Dataset")
    parser.add_argument("--dataset", type=str, required=True, help="Pfad zur Eingabedatei (CSV)")
    parser.add_argument("--target", type=str, required=True, help="Zielspalte (z. B. wer_tiny)")
    parser.add_argument("--out_dir", type=str, default="results/model_comparisons", help="Ausgabeverzeichnis")
    parser.add_argument("--trials", type=int, default=15, help="Anzahl der Optuna-Trials pro Modell")
