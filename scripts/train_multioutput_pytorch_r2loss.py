# scripts/train_multioutput_pytorch_r2loss.py
...
def r2_loss(y_pred, y_true):
    """
    Differenzierbare, negative R²-Loss-Funktion:
    1 - mean(R²_i)  -> wird minimiert
    """
    eps = 1e-8
...
def main():
    parser = argparse.ArgumentParser(description="Trainiere Multi-Output PyTorch Modell (R²-Loss)")
    ...
    print(f"=== Multi-Output PyTorch Training (R²-Loss) auf {args.dataset} ===")
    ...
    for epoch in range(args.epochs):
        ...
        if (epoch + 1) % 10 == 0 or epoch == 0:
            ...
            print(f"Epoche {epoch + 1}/{args.epochs} | Train R2-Loss={train_loss:.4f} | Val MSE={val_loss:.4f}")
    ...
    results_df.to_csv(os.path.join(args.out_dir, "pytorch_multioutput_r2loss_results.csv"), index=False)
    torch.save(model.state_dict(), os.path.join(args.out_dir, "pytorch_multioutput_r2loss_model.pt"))
