...
import joblib  # NEU: für Scaler-Speicherung
...
def eval_on_loader(model, loader, device):
    ...
    with torch.no_grad():
        for xb, yb in loader:
            xb = xb.to(device)
            yb = yb.numpy()  # y bleibt auf CPU, ist ok
...
# ------------------------------------------------------
def main():
    parser = argparse.ArgumentParser(description="Train Multi-Output MLP on CV23 (SigMOS+WavLM) für WER-Regression")
    parser.add_argument("--dataset", required=True, help="CSV mit Features + Targets")
    parser.add_argument("--out_dir", required=True, help="Ausgabeverzeichnis (z.B. models/regression)")
    ...
    # Scaler als joblib (sklearn-Objekt) speichern
    scaler_path = os.path.join(args.out_dir, "scaler.pkl")
    joblib.dump(scaler, scaler_path)
    print(f"Scaler gespeichert unter: {scaler_path}")

    # Hyperparameter-Grid (kannst du bei Bedarf später auch fixieren)
    grid = []
    hidden_sets = [
        [512, 256, 128],
    ]
    ...
    df_gs.to_csv(os.path.join(args.out_dir, "mlp_reg_hparam_search_results.csv"), index=False)
    ...
    model_path = os.path.join(args.out_dir, "mlp_regression_multiwer.pt")
    torch.save(model.state_dict(), model_path)
    print(f"Modell gespeichert unter: {model_path}")

    with open(os.path.join(args.out_dir, "mlp_regression_config_and_metrics.json"), "w") as f:
        json.dump(
            {
    ...
    pd.DataFrame([test_metrics]).to_csv(
        os.path.join(args.out_dir, "mlp_regression_test_metrics.csv"),
        index=False,
    )
