...
def objective(trial, model_name, X_train, X_val, y_train, y_val):
    if model_name == "lightgbm":
        ...
        params = {"alpha": trial.suggest_float("alpha", 1e-4, 1.0, log=True), "max_iter": 10000}
...
def main():
    parser = argparse.ArgumentParser(description="Vergleiche Modelle auf SigMOS+WavLM clean+noisy Dataset")
    parser.add_argument("--dataset", type=str, required=True)
    parser.add_argument("--target", type=str, required=True)
    parser.add_argument("--out_dir", type=str, default="results/model_comparisons_clean_and_noisy")
